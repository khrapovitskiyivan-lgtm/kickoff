...
1. Read the project context: `CLAUDE.md` (stack, conventions, roadmap / open tasks), stack & config files (`package.json`, `requirements.txt`, `pyproject.toml`, `go.mod`, `.git`, CI files, `Dockerfile`, tests dir), and **inventory installed tooling** by running `claude plugin list` + `claude mcp list` and noting the available skills — so nothing already present gets re-recommended.
