...
25. **Paid-API cost abuse** — **a budget alert is detection, not a cap**: provider alerts notify, they do not stop spending, and they usually lag hours behind usage. The real ceilings are a **prepaid balance with auto-recharge turned off** and the provider's own **hard spend limit** where one exists. In your code: require auth before any paid call (the open demo route is how this happens), cap per-user daily calls and `max_tokens` per call, and cache repeats. Note the trade-off before shipping a global fail-closed ceiling: one abuser can burn it and deny everyone.
